...
impl Account {
    ...
    fn transfer_money(
        ...
        let mut pay_to_user_account = Account::from_row(pay_to_user_row)?; // dont need to do this I think
        self.balance -= amount;
        ...
        d.edit_row(
            d.get_row_index(pay_to_user_row).unwrap(),
            &pay_to_user_new_row,
        );
        d.edit_row(d.get_row_index(self_user_row).unwrap(), &self_user_new_row);
        println!("{:?}", d);
